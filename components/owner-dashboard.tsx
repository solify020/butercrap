...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (!currentUser) {
        // Clear cookies and redirect if no user
        handleSignOut(false)
        return
      }
      ...
      if (!authCookie || !roleCookie || roleCookie !== "owner") {
        // If cookies are missing or user is not an owner, sign out and redirect
        handleSignOut(false)
        return
      }
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <Button
            ...
            className="rounded-md px-4 py-2 h-auto ios-zoom-in force-animation"
            style={{ animationDelay: "250ms", opacity: dashboardReady ? 1 : 0 }}
          >
