...
export default function ContentCalendarPage() {
  ...
  const updateSelectedDateEvents = (selectedDate: Date | null, eventsList: CalendarEvent[] = events) => {
    if (!selectedDate) return

    try {
      const dateString = format(selectedDate, "yyyy-MM-dd")
      const filteredEvents = eventsList.filter((event) => {
        // Safely check if the event date starts with the selected date string
        return event.date && typeof event.date === "string" && event.date.startsWith(dateString)
      })
      setSelectedDateEvents(filteredEvents)
    } catch (error) {
      console.error("Error updating selected date events:", error)
  ...
  const handleAddEvent = async () => {
    ...
    setIsSaving(true)
    try {
      // Create event object with safe date formatting
    ...
    try {
      ...
      return (
        <div className="relative w-full h-full">
          <div className={`absolute inset-0 flex items-center justify-center ${isCurrentDay ? "font-bold" : ""}`}>
            {day.getDate()}
          </div>
          {hasEvents && (
  ...
  // Format date for input fields safely
    ...
    <div className="min-h-screen animate-fadeIn">
      <header className="p-6 bg-[#333333] shadow-md flex justify-between items-center">
        ...
        <Button
          ...
          className="bg-[#5a5a5a] hover:bg-[#666666] text-white transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-md transform hover:translate-y-[-2px]"
      ...
      <main className="container mx-auto px-6 py-6">
        <div className="grid grid-cols-1 md:grid-cols-7 gap-6">
          ...
          <Card className="bg-[#333333] border-[#666666]/30 text-white md:col-span-5 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-lg hover:border-[#666666]/40 transform hover:translate-y-[-2px]">
              ...
              {currentMonth && isClient && (
                <Calendar
                    ...
                    Day: ({ day, ...props }) => {
                      try {
                        return (
                          <button
                            {...props}
                            className={`relative h-10 w-full p-0 font-normal aria-selected:opacity-100 hover:bg-[#444444]/20 rounded-md ${
                  ...
                  classNames={{
                    ...
                    day_selected: "bg-[#5a5a5a]/30 text-white font-bold",
                    nav_button: "text-white hover:bg-[#444444]/20",
          ...
          <Card className="bg-[#333333] border-[#666666]/30 text-white md:col-span-2 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-lg hover:border-[#666666]/40 transform hover:translate-y-[-2px]">
            <CardHeader>
        ...
        <DialogContent className="bg-[#333333] text-white border-[#666666]/30">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isSaving}
              className="bg-[#5a5a5a] hover:bg-[#666666] text-white"
              style={{ backgroundColor: "#5a5a5a", color: "white" }}
            >
              {isSaving ? (
                <>
          ...
          {viewingEvent && (
            <>
              <DialogHeader>
                ...
                <DialogDescription className="text-gray-400">
                  {viewingEvent && viewingEvent.date
